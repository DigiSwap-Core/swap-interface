import { Currency, ETHER, Token } from '@digiswap-core/sdk-v1'
...
const StyledBnbLogo = styled.img<{ size: string }>`
  ...
  border-radius: 24px;
`
const StyledCoinLogo = styled.img<{ size: string }>`
width: ${({ size }) => size};
height: ${({ size }) => size};
box-shadow: 0px 6px 10px rgba(0, 0, 0, 0.075);
border-radius: 24px;
...
export default function CurrencyLogo({
  ...
  return (currency as any)?.symbol ? (
    ...
    <StyledCoinLogo src={`/images/coins/${currency?.symbol ?? "token"}.png`} size={size} style={style} />
  ) : (
